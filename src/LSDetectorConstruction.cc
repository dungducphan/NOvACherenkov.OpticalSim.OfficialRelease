...
G4VPhysicalVolume * LSDetectorConstruction::Construct() {
  ...
  detectorPhy = new G4PVPlacement(rotm1, G4ThreeVector(-85*cm, 0, -1.2*m - halfTubeLengthAddition), detectorLog, "DetectorPhy", worldLog, false, 0);
  mirrorPhy = new G4PVPlacement(rotm2, G4ThreeVector(0, 0, -1.2*m - halfTubeLengthAddition), mirrorLog, "MirrorPhy", worldLog, false, 0);
  conePhy = new G4PVPlacement(rotm1, G4ThreeVector(-52*cm, 0, -1.2*m - halfTubeLengthAddition), coneLog, "ConePhy", worldLog, false, 0);
  ...
  G4LogicalSkinSurface* mirrorSurface = new G4LogicalSkinSurface("MirrorSurface", mirrorLog, opMirrorSurface);
  ...
  G4LogicalSkinSurface* coneSurface = new G4LogicalSkinSurface("ConeSurface", coneLog, opConeSurface);
  ...
  G4double *ephoton_mirror      = (G4double*) malloc(wavelength_reflec.size()* sizeof(G4double));
  G4double *reflectivity_mirror = (G4double*) malloc(wavelength_reflec.size()* sizeof(G4double));
  G4double *efficiency_mirror   = (G4double*) malloc(wavelength_reflec.size()* sizeof(G4double));

  for (unsigned short int idx = 0; idx < wavelength_reflec.size(); idx++) {
    ephoton_mirror[idx] = (1242.38 / wavelength_reflec.at(idx))*eV;
    reflectivity_mirror[idx] = (reflec.at(idx))/100.;
    efficiency_mirror[idx] = 0.;
  }
